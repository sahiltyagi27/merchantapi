...
describe('Update Web Template', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/web-templates/123-abc`, {
                json: true,
                headers: {
    ...
    it('should delete the document when all validation passes', async () => {
        const response = await request.patch(`${helpers.API_URL}/api/v1/web-templates/${sampleWebTemplates._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            },
            body: { userMerchants: [ merchantID ],
                updatedWebTemplate: { isEnabled: false }}
        });
