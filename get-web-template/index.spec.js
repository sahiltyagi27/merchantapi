...
describe('Get Web Template', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/web-templates/123-abc`, {
                json: true,
                headers: {
    ...
    it('should return the document when all validation passes', async () => {
        const response = await request.get(`${helpers.API_URL}/api/v1/web-templates/${sampleWebTemplates._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
        ...
        expect(response._id).to.equal(sampleWebTemplates._id);
    });
    after(async () => {
        const collection = await getMongodbCollection('Merchants');
